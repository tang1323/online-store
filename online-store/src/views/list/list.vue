<template>
    <div id="wrapper">
        ...
        <div class="main cle">
<!--          proNum="proNum"就是101行传递到43行，再拿在这里返回给页面显示-->
    ...
    export default {
        data () {
            return {
                ...
                isObject:true,
                ordering: '-add_time',
                proNum: 0, //商品数量，这个count是与drf里的count拿过来的，就是为了获取多少数量，在101行
                curLoc: [], //当前位置
                priceRange: [], //价格区间
                pricemin: '', //价格最低
                pricemax: '', //价格最高
        ...
        //这是标题栏和搜索框共用的一个组件，在这里刚形成，先被head.vue引用，再被index.js引用
        created () {
            this.getAllData ();
        },
        watch: {
        ...
        methods: {
            getAllData () {
              //params,这个可以根据自己的参数来判断是哪一个，在index.js
                console.log(this.$route.params);
                if(this.$route.params.id){
                    this.top_category = this.$route.params.id;
                    //如果接收到的是id那getMenu就会到里面找
            ...
            getListData() {
                if(this.pageType=='search'){
                  getGoods({
                    ...
                    // 这个count是与drf里的count拿过来的，就是为了获取多少数量
                  ...
                  getGoods({
                    //这个要跟后端的views里面的页码一致：page_query_param = "page"
                    page: this.curPage, //当前页码
                    //这个top_category就是首页的标题栏，这个是一级分类， 点击一级分类后要获取到二级以下的所有商品，
                    // 这里先要获取二级分类，再由二级分类获取里面的所有的商品
                    //就是在filter要用不一样的逻辑来做了，与filter里的top_category保持一致
