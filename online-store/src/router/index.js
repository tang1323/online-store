        ...
        children: [
          {
            //这个就是在head.vue进一步的定义
            //这就是商品列表页的
            //这里是同样的component: list，所以用的是同一种组件，这个组件刚形成的时候是在list/list.vue
            //这里的参数是不一样，它们能区别'list/:id'，
            //这个语句是在list.vue的79行：console.log(this.$route.params)
            //这个params能根据参数是id还是keyword进行判断
            path: 'list/:id',
            name: 'list',
            component: list,
            meta: {
          ...
          },
          {
            ...
            //这就是搜索的结果页
            //这里是同样的component: list，所以用的是同一种组件，这个组件刚形成的时候是在list/list.vue
            //这里的参数是不一样，它们能区别'search/:keyword'
            //这个语句是在list.vue的79行：console.log(this.$route.params)
            //这个params能根据参数是id还是keyword进行判断
            path: 'search/:keyword',
            name: 'search',
